...
void led_run_rl(void)
{
    // Sequential Blink: 15 -> 14 -> 13 -> 12
    for(int pin = 15; pin >= 12; pin--)
    {
        GPIOD->ODR |= BV(pin);    // Turn LED ON
        DelayMs(500);             // Wait (Adjusted from 5000 to 200 for visibility)
        GPIOD->ODR &= ~BV(pin);   // Turn LED OFF
    }
}
